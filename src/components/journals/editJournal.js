...
export default () => {
    ...
    useEffect(() => {
        JournalRepository.getJournalTransactionsbyId(journalId)
        .then((data) => {
    ...
    return (
        ...
        <form className="journal_form">
                <fieldset className="journal_form_fields">
                    ...
                    <input id="have_field" type="number" value={form.have} onChange={(event) => {
                        let copy={...form}
                        copy.have = parseFloat(event.target.value)
                        setForm(copy)
                    }}/>
                    <label>How much would you have liked to save?</label>
                    <input id="save_field" type="number" value={form.save} onChange={(event) => {
                        let copy={...form}
                        copy.save = parseFloat(event.target.value)
                        setForm(copy)
                    }}/>
                    <label>Did you spend more or less than what you planned on, why?</label>
                    <input id="spend_field" type="text" value={form.spend} onChange={(event) => {
                        let copy={...form}
                        copy.spend = event.target.value
                        setForm(copy)
                    }}/>
                    <label>What are you happy about in this snapshot? What can you improve?</label>
                    <input id="reflection_field" type="text" value={form.reflection} onChange={(event) => {
                        let copy={...form}
                        copy.reflection = event.target.value
                        setForm(copy)
                    }}/>
                </fieldset>
                <button onClick={(event) => {sendUpdatedJournal(event)}}>Update Journal</button>
            </form>
